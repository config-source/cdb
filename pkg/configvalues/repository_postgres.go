...
func getConfigurationRecursively(ctx context.Context, r *PostgresRepository, envRepo environments.Repository, environmentID int, excludedKeys []string) ([]ConfigValue, error) {
	env, err := envRepo.GetEnvironment(ctx, environmentID)
	if err != nil {
		return nil, err
	}
	...
	if env.PromotesToID != nil {
		parentValues, err := getConfigurationRecursively(ctx, r, envRepo, *env.PromotesToID, append(excludedKeys, getAllKeys(immediateValues)...))
...
func (r *PostgresRepository) GetConfiguration(ctx context.Context, environmentID int) ([]ConfigValue, error) {
	...
	envRepo := environments.NewRepository(r.log, r.pool)
	env, err := envRepo.GetEnvironment(ctx, environmentID)
	if err != nil {
		return nil, err
	}
	...
	if env.PromotesToID != nil {
		parentValues, err := getConfigurationRecursively(ctx, r, envRepo, *env.PromotesToID, getAllKeys(immediateValues))
		return append(immediateValues, parentValues...), err
	}
...
func (r *PostgresRepository) GetConfigurationValue(ctx context.Context, environmentID int, key string) (*ConfigValue, error) {
	...
	if errors.Is(err, ErrNotFound) {
		...
		if env.PromotesToID != nil {
			parent, err := envRepo.GetEnvironment(ctx, *env.PromotesToID)
