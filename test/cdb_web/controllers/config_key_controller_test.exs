defmodule CdbWeb.ConfigKeyControllerTest do
  ...
  describe "index" do
    test "lists all config_keys", %{conn: conn} do
      conn = get(conn, ~p"/config_keys")
  ...
  describe "new config_key" do
    test "renders form", %{conn: conn} do
      conn = get(conn, ~p"/config_keys/new")
  ...
  describe "create config_key" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, ~p"/config_keys", config_key: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == ~p"/config_keys/#{id}"

      conn = get(conn, ~p"/config_keys/#{id}")
      assert html_response(conn, 200) =~ "Config key #{id}"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/config_keys", config_key: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Config key"
    end
  end
  ...
  describe "edit config_key" do
    ...
    test "renders form for editing chosen config_key", %{conn: conn, config_key: config_key} do
      conn = get(conn, ~p"/config_keys/#{config_key}/edit")
  ...
  describe "update config_key" do
    ...
    test "redirects when data is valid", %{conn: conn, config_key: config_key} do
      conn = put(conn, ~p"/config_keys/#{config_key}", config_key: @update_attrs)
      assert redirected_to(conn) == ~p"/config_keys/#{config_key}"

      conn = get(conn, ~p"/config_keys/#{config_key}")
      assert html_response(conn, 200) =~ "some updated name"
    end

    test "renders errors when data is invalid", %{conn: conn, config_key: config_key} do
      conn = put(conn, ~p"/config_keys/#{config_key}", config_key: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Config key"
    end
  end
  ...
  describe "delete config_key" do
    ...
    test "deletes chosen config_key", %{conn: conn, config_key: config_key} do
      conn = delete(conn, ~p"/config_keys/#{config_key}")
      assert redirected_to(conn) == ~p"/config_keys"

      assert_error_sent 404, fn ->
        get(conn, ~p"/config_keys/#{config_key}")
      end
    end
  end
